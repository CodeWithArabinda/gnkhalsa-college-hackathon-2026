...
import { Skill, SkillNames } from "@portfolio1/data/constants";
import { usePortfolio } from "@portfolio1/contexts/PortfolioContext";
import { sleep } from "@portfolio1/lib/utils";
import { useMediaQuery } from "@portfolio1/hooks/use-media-query";
...
const KeyboardScene = ({ maxDpr }: { maxDpr: number }) => {
  ...
  const updateKeyboardTransform = async () => {
    ...
    keycaps.forEach(async (keycap, idx) => {
      ...
      gsap.fromTo(
        keycap.position,
        { y: 100 },
        { y: 0, duration: 0.6, delay: 0.1, ease: "bounce.out" }
      );
    });
  };
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    let floatKeyboard: gsap.core.Tween | undefined;

    const kbd = splineApp.findObjectByName("keyboard");

    if (kbd) {
      // Gentle floating animation that preserves the visible isometric orientation
      floatKeyboard = gsap.to(kbd.position, {
        y: "+=15",
        duration: 3,
        repeat: -1,
        yoyo: true,
  ...
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <Spline
        className="w-full h-full fixed inset-0 top-0 left-0 z-0"
        ref={splineContainer}
