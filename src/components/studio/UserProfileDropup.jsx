...
export default function UserProfileDropup({ onOpenSettings }) {
  ...
  return (
    <div ref={dropupRef} className="relative w-full p-2 border-t border-slate-200 bg-white select-none">
      
      {/* Expandable Menu Popover (Expands Upwards above Sidebar Trigger) */}
      {isOpen && (
        <div className="absolute bottom-16 left-2 w-[210px] bg-white border border-slate-200 shadow-2xl rounded-2xl p-2 z-50 space-y-1 font-sans text-xs animate-in fade-in slide-in-from-bottom-2 duration-150">
          
          {/* Header Info */}
          <div className="px-2.5 py-2 border-b border-slate-100 flex items-center gap-2.5">
          ...
          <div className="pt-1 space-y-0.5 font-sans">
            <button
              ...
              className="w-full flex items-center gap-2 px-2.5 py-2 hover:bg-slate-100 rounded-xl text-left font-semibold text-amber-600 text-xs transition-colors"
            >
            ...
            <button
              ...
              className="w-full flex items-center gap-2 px-2.5 py-2 hover:bg-slate-100 rounded-xl text-left text-slate-700 text-xs transition-colors"
            >
      ...
      {/* Pure White Docked Profile Card Trigger */}
      <button
        type="button"
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center justify-between p-2.5 rounded-xl bg-white border border-slate-200 hover:bg-slate-50 shadow-sm transition-colors cursor-pointer text-slate-900"
      >
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="text-left font-sans min-w-0">
            <p className="text-xs font-bold text-slate-900 truncate block text-left leading-tight">
              {userName}
            </p>
            <p className="text-[10px] font-semibold text-slate-500 block text-left leading-tight">
              Free Plan
            </p>
          </div>
        </div>

        <Settings className="w-4 h-4 text-slate-400 hover:text-slate-700 shrink-0 transition-colors" />
      </button>
