...
export function usePerfProfile(): PerfProfile {
  ...
  return React.useMemo<PerfProfile>(() => {
    ...
    // disable3D only when the user has EXPLICITLY opted out via the in-app
    // toggle (motionPref === "off") or has Data Saver on.
    // The OS `prefers-reduced-motion` setting is respected for decorative
    // effects (particles, elastic cursor) but must NOT silently remove the
    // headline 3D keyboard — many Windows users have that flag set by default
    // without realising it would hide the main visual feature of the site.
    const disable3D = motionPref === "off" || saveData;
